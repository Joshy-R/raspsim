...
static void pySetAlignedBuf(void *dst, const void *src, size_t size) {
    ...
    if ((((uintptr_t)src) % Raspsim::getPageSize()) + size >
        Raspsim::getPageSize()) {
...
class AddrRef {
   public:
    ...
    operator py::bytes() { return py::bytes((char *)addr, 1); }

    AddrRef &operator=(W32 value) {
        *(W32 *)addr = value;
        return *this;
    }
    AddrRef &operator=(W64 value) {
        *(W64 *)addr = value;
        return *this;
    }
    AddrRef &operator=(byte value) {
        *(byte *)addr = value;
        return *this;
    }
    AddrRef &operator=(float value) {
        *(float *)addr = value;
        return *this;
    }
    AddrRef &operator=(double value) {
        *(double *)addr = value;
        return *this;
    }

    AddrRef operator[](Waddr offset) const {
        return AddrRef((byte *)addr + offset);
    }
    ...
    py::bytes read(Waddr size = 1) { return py::bytes((char *)addr, size); }
...
class PyRaspsim : public Raspsim {
   public:
    ...
    AddrRef mapPage(Waddr start, Prot prot);
...
AddrRef PyRaspsim::mapPage(Waddr start, Prot prot) {
    return AddrRef(Raspsim::mapPage(start, static_cast<int>(prot)));
}

AddrRef PyRaspsim::memmap(Waddr start, Prot prot, Waddr length,
                          py::bytes data) {
    ...
    byte *startptr = nullptr;
    W64 nprocessed = 0;
    for (Waddr pageaddr = start & ~(getPageSize() - 1);
         pageaddr < start + length; pageaddr += getPageSize()) {
        byte *pageptr = getMappedPage(pageaddr);
        if (!pageptr) {
            pageptr = mapPage(pageaddr, prot);
        } else if (getPageProtection(pageptr) != static_cast<int>(prot)) {
            std::stringstream ss;
            ss << "Page @ " << (void *)pageaddr
               << " already mapped with permissions "
               << static_cast<Prot>(getPageProtection(pageptr))
               << " instead of " << prot;
            throw RaspsimException(strdup(ss.str().c_str()));
        }

        if (!startptr) {
            startptr = pageptr + (start & (getPageSize() - 1));
        }

        if (raw && nprocessed < bytes.size()) {
            W64 chunklength = std::min(
                bytes.size() - nprocessed,
                getPageSize() - ((start + nprocessed) & (getPageSize() - 1)));
            memcpy(startptr + nprocessed, raw + nprocessed, chunklength);
        }
    }
    return AddrRef(startptr);
}
